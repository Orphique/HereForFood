...
// --- photo attachment for chat (real vision logging) ------------------------
let chatPhoto = null; // data: URI of the attached photo

$('chatPhotoBtn').onclick = () => $('chatPhotoInput').click();
$('chatPhotoClear').onclick = () => clearChatPhoto();
$('chatPhotoInput').onchange = () => {
  const file = $('chatPhotoInput').files[0];
  if (!file) return;
  if (file.size > 4 * 1024 * 1024) { alert('Photo is too large (max 4 MB).'); $('chatPhotoInput').value = ''; return; }
  const reader = new FileReader();
  reader.onload = () => {
    chatPhoto = reader.result;
    $('chatPhotoImg').src = chatPhoto;
...
function clearChatPhoto() {
  ...
  $('chatPhotoImg').removeAttribute('src');
}

async function sendMessage(text) {
  if (!text.trim() && !chatPhoto) return;
  addMsg('user', chatPhoto ? `📷 ${text || '(photo of my meal)'}` : text);
  $('chatInput').value = '';
  ...
  try {
    const res = await api('/chat', {
      method: 'POST',
      body: JSON.stringify({ message: text, ...(photo ? { image: photo } : {}) }),
    });
    thinking.remove();
    addMsg('bot', res.reply);
    if (res.dashboard) renderDashboard(res.dashboard); // keep dashboard live
    // The coach can change settings now — keep the app in sync with what it did.
    if (res.profile) {
      state.profile = res.profile;
      $('goalBadge').textContent = GOAL_LABEL[res.profile.goal] || 'No goal set';
      if (!$('page-settings').classList.contains('hidden')) fillSettings();
    }
    // The coach can also open a page for the user.
...
// Close dropdown when selecting a page tab on mobile
...
// Optional: Close menu if user clicks anywhere outside topbar
