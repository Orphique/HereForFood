...
router.put('/profile', async (req, res, next) => {
  try {
    const allowed = [
      'name', 'age', 'gender', 'weightKg', 'heightCm', 'activityLevel', 'medicalCondition',
      'goal', 'calorieGoal', 'allergies', 'dietaryRestrictions', 'preferences', 'budgetPerMeal',
    ];
    const patch = {};
    for (const k of allowed) if (k in req.body) patch[k] = req.body[k];

    // Only compute a calorie goal if the user didn't set one explicitly here.
    const userSetCalorie = 'calorieGoal' in req.body && req.body.calorieGoal != null;
    if (!userSetCalorie) {
      const current = await store.getOrCreateUser(userId(req));
      const merged = { ...current.profile, ...patch };
      // Preferred: personalise from body metrics + activity (updated requirements).
      const computed = recommendedCalories(merged);
      if (computed != null) {
        patch.calorieGoal = computed;
      } else if (patch.goal && current.profile.calorieGoal == null) {
        // Fallback flat default when we don't have body metrics yet.
        patch.calorieGoal = { lose_weight: 1600, maintain: 2000, gain_weight: 2600,
          build_muscle: 2500, eat_healthier: 2000, manage_diet: 1800, track_food: 2000,
          doctor_recommendation: 1900, elderly_energy: 1900 }[patch.goal] || 2000;
      }
    }

    const profile = await store.updateProfile(userId(req), patch);
    res.json({ ...profile, bmi: bmi(profile), bmiCategory: bmiCategory(bmi(profile)) });
  } catch (e) { next(e); }
});
...
router.post('/chat', async (req, res, next) => {
  try {
    ...
    res.json({ reply: out.reply, toolTrace: out.toolTrace, dashboard: await computeDashboard(user.id) });
  } catch (e) { next(e); }
});
